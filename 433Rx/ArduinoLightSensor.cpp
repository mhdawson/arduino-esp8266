...
ArduinoLightSensor::ArduinoLightSensor(PubSubClient* client, char* topic) :
...
void ArduinoLightSensor::publishTopic(int messageNum, Message* message,
...
void ArduinoLightSensor::getMessageText(int messageNum, Message* message,
                                        char* buffer, int maxLength) {
  char* bytes = (char*) &message->code;
  int lightValue  = std::min(bytes[2] * 256 + bytes[1], 1000);
