...
ArduinoTHSensor2::ArduinoTHSensor2(PubSubClient* client, char* topic) :
...
void ArduinoTHSensor2::publishTopic(int messageNum, Message* message,
                                    char* buffer, int maxLength) {
  char* bytes = (char*) message->longCode;
  if (messageNum == 0) {
    snprintf(&buffer[strlen(buffer)], maxLength - strlen(buffer),
             "/%x/temp", getDeviceId(bytes));
  } else {
    snprintf(&buffer[strlen(buffer)], maxLength - strlen(buffer),
            "/%x/humidity", getDeviceId(bytes));
  }
};


void ArduinoTHSensor2::getMessageText(int messageNum, Message* message,
                                      char* buffer, int maxLength) {
  char* bytes = (char*) &message->longCode;
  if (messageNum == 0) {
