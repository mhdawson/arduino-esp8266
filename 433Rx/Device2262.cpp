...
Device2262::Device2262(PubSubClient* client, char* topic) : Device(), MqttDevice(client, topic) {
   syncFound = false;
   bitCount = 0;
   code = 0;
      ...
      if (BITS_IN_MESSAGE_2262 == bitCount) {
         for (int i=0;i<BITS_IN_MESSAGE_2262;i=i+2) {
             ...
             if ((ONE_PULSE_MIN_LENGTH < durations[i]) && (ONE_PULSE_MIN_LENGTH < durations[i+1])) {
                ...
                // bit is 0, we only code devices to use high or floating so this is invalid
                syncFound = false;
                return;
             } else if ((ONE_PULSE_MIN_LENGTH >= durations[i]) && (ONE_PULSE_MIN_LENGTH < durations[i+1])) {
...
void Device2262::publishTopic(Message* message, char* buffer, int maxLength) {
   snprintf(&buffer[strlen(buffer)], maxLength - strlen(buffer), "/%x", message->code);
}
